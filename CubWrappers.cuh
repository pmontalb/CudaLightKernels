...
EXTERN_C
{
	...
	EXPORT int _MinRaw(double& min, const ptr_t x, const unsigned size, const MemorySpace memorySpace, const MathDomain mathDomain)
	{
		return _Min(min, MemoryBuffer(x, size, memorySpace, mathDomain));
	}

	EXPORT int _Max(double& max, const MemoryBuffer x);
	EXPORT int _MaxRaw(double& max, const ptr_t x, const unsigned size, const MemorySpace memorySpace, const MathDomain mathDomain)
	{
